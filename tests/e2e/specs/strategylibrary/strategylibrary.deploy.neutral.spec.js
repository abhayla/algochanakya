/**
 * Strategy Library Deployment Tests - Neutral Strategies
 *
 * Tests deployment of all neutral strategies from Strategy Library to Strategy Builder:
 * - Iron Condor (4 legs)
 * - Iron Butterfly (4 legs)
 * - Short Straddle (2 legs)
...
test.describe('Strategy Library Deploy - Neutral Strategies @deploy @neutral', () => {
  ...
  // Generate tests for each neutral strategy
  for (const strategyName of NEUTRAL_STRATEGIES) {
    ...
    test.describe(`${template.displayName}`, () => {
      ...
      test(`should verify ${template.displayName} leg fields after deployment`, async ({ authenticatedPage }) => {
        ...
        for (let i = 0; i < template.legs.length; i++) {
          ...
          // but the data should still be present (entry price populated indicates leg is valid)
        ...
        // At least some legs should have visible strikes (typically ATM or near-ATM)
