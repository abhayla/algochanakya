    ...
    <div class="header-left">
      ...
      <div class="index-prices" data-testid="kite-header-index-prices">
        <div class="index-item" data-testid="kite-header-index-nifty50">
          ...
          <span
            :class="['index-price', (indexTicks.nifty50?.change || 0) >= 0 ? 'up' : 'down']"
            data-testid="kite-header-index-value-nifty50"
          >
            {{ formatNumber(indexTicks.nifty50?.ltp) }}
          </span>
          <span :class="['index-change', (indexTicks.nifty50?.change || 0) >= 0 ? 'up' : 'down']">
            {{ formatChange(indexTicks.nifty50?.change) }} ({{
              formatPct(indexTicks.nifty50?.change_percent)
            }})
          </span>
        </div>
        <div class="index-item" data-testid="kite-header-index-sensex">
          ...
          <span
            :class="['index-price', (indexTicks.sensex?.change || 0) >= 0 ? 'up' : 'down']"
            data-testid="kite-header-index-value-sensex"
          >
            {{ formatNumber(indexTicks.sensex?.ltp) }}
          </span>
          <span :class="['index-change', (indexTicks.sensex?.change || 0) >= 0 ? 'up' : 'down']">
            {{ formatChange(indexTicks.sensex?.change) }} ({{
              formatPct(indexTicks.sensex?.change_percent)
            }})
          </span>
        </div>
      </div>
    ...
    <div class="header-right">
      ...
      <div class="header-actions">
        ...
        <div
          ...
          :style="{ color: marketStatus.color, borderColor: marketStatus.color + '40' }"
          data-testid="kite-header-market-status"
        >
        ...
        <!-- User Menu -->
        <div class="user-menu" data-testid="kite-header-user-menu" @click="toggleUserDropdown">
        ...
        <div v-if="showUserDropdown" class="user-dropdown" data-testid="kite-header-user-dropdown">
          ...
          <button
            ...
            data-testid="kite-header-logout-button"
            @click="logout"
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
<script setup>
  ...
  const allNavItems = [
    ...
    { path: '/optionchain', label: 'Option Chain' },
    {
      ...
      label: 'OFO',
      feature: 'ofo',
      title: 'Options For Options — Find optimal strategy combinations',
    },
    { path: '/strategy', label: 'Strategy' },
    { path: '/positions', label: 'Positions' },
    { path: '/autopilot', label: 'AutoPilot', feature: 'autopilot', icon: 'robot' },
    { path: '/ai/settings', label: 'AI Settings', feature: 'ai', icon: 'brain' },
    { path: '/watchlist', label: 'Watchlist', feature: 'watchlist' },
  ]
  const navItems = computed(() =>
    allNavItems.filter((i) => !i.feature || isModuleEnabled(i.feature))
  )
  ...
  const marketStatus = computed(() => {
    ...
    if (timeInMinutes >= 540 && timeInMinutes < 555)
      return { label: 'Pre-Market', color: '#f59e0b', isOpen: false }

    // Market open: 9:15 - 15:30
    if (timeInMinutes >= 555 && timeInMinutes <= 930)
      return { label: 'Market Open', color: '#15803d', isOpen: true }

    // Post-market: 15:30 - 16:00
    if (timeInMinutes > 930 && timeInMinutes <= 960)
      return { label: 'Post-Market', color: '#f59e0b', isOpen: false }
...
<style scoped>
  ...
  .index-price.down {
    color: #d43f3a;
  } /* WCAG AA compliant (4.59:1 contrast) */
