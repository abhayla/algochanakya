<script setup>
  ...
  // Broker option keys map to feature flag keys (sourceKey: kite=zerodha, smartapi=angelone)
  const allSourceCardOptions = [
    {
      value: 'platform',
      label: 'Platform Default',
      description: 'Shared AngelOne + Upstox connection with automatic failover.',
    },
    {
      value: 'smartapi',
      label: 'AngelOne SmartAPI',
      description: 'Free real-time data with 20-level market depth.',
      flag: 'angelone',
    },
    {
      value: 'kite',
      label: 'Zerodha Kite',
      description: 'Uses your Kite login. Requires daily re-login (~6 AM IST).',
      flag: 'zerodha',
    },
    {
      value: 'upstox',
      label: 'Upstox',
      description: 'Free market data via Upstox API.',
      flag: 'upstox',
    },
    { value: 'dhan', label: 'Dhan', description: 'Free market data via Dhan API.', flag: 'dhan' },
    {
      value: 'fyers',
      label: 'Fyers',
      description: 'Free market data via Fyers API.',
      flag: 'fyers',
    },
    {
      value: 'paytm',
      label: 'Paytm Money',
      description: 'Free market data via Paytm Money API.',
      flag: 'paytm',
    },
  ]
  const sourceCardOptions = computed(() =>
    allSourceCardOptions.filter((o) => !o.flag || isBrokerEnabled(o.flag))
  )
  ...
  // Map order broker option values (kite, smartapi, ...) to feature flag keys
  const orderBrokerFlagMap = {
    kite: 'zerodha',
    zerodha: 'zerodha',
    smartapi: 'angelone',
    angel: 'angelone',
  ...
  const filteredOrderBrokerOptions = computed(() =>
    (store.orderBrokerOptions || []).filter((o) => {
      const flagKey = orderBrokerFlagMap[String(o.value).toLowerCase()]
      return !flagKey || isBrokerEnabled(flagKey)
    })
  )
  ...
  <div class="broker-settings" data-testid="settings-broker-section">
    ...
    <div class="broker-setting-group">
      <div class="group-header">
        ...
        <p class="group-subtitle">
          Choose where live quotes and OHLC data come from. Platform Default uses our shared
          AngelOne + Upstox connections with automatic failover.
        </p>
      </div>
        ...
        >
          ...
          <div class="source-card-content">
            <div class="source-card-header">
              ...
              <span v-if="localMarketDataSource === option.value" class="badge badge-current"
                >Current</span
              >
              <span v-else-if="option.value === 'platform'" class="badge badge-available"
                >Available</span
              >
              <span
                v-else-if="store.isBrokerConfigured(option.value)"
                class="badge badge-configured"
                >Configured</span
              >
      ...
      <p
        v-if="
          localMarketDataSource !== 'platform' && !store.isBrokerConfigured(localMarketDataSource)
        "
        class="source-warning"
        data-testid="settings-source-warning"
      >
        Configure
        {{ store.marketDataSourceOptions.find((o) => o.value === localMarketDataSource)?.label }}
        credentials below first — data will use platform default until connected.
      </p>
    </div>
    ...
    <div class="broker-setting-group">
      <div class="group-header">
        ...
        <p class="group-subtitle">
          All orders are placed through your personal broker account (SEBI-compliant). Configure
          credentials in your broker's OAuth settings.
        </p>
      </div>
      ...
      <div class="setting-row">
        ...
        <div class="setting-control">
          ...
          >
            ...
            <option
              v-for="option in filteredOrderBrokerOptions"
              :key="option.value"
              :value="option.value"
            >
              {{ option.label }}
