 ...
 * KiteSettings
 *
 * OAuth connection management + Tier 3 API credential storage for Zerodha.
 * Section 1: OAuth connect/disconnect (existing behavior preserved).
 * Section 2: Save user's own Kite Connect api_key + api_secret.
 */
...
async function loadCredentials() {
  ...
  try {
    saved.value = await zerodhaCredentials.getCredentials()
  } catch {
...
async function handleCredsSave() {
  ...
  try {
    saved.value = await zerodhaCredentials.storeCredentials({
...
async function handleCredsDelete() {
  if (!confirm('Remove saved Kite Connect API credentials?')) return
  credsDeleting.value = true
  credsError.value = null
  try {
    await zerodhaCredentials.deleteCredentials()
...
<template>
  <div class="kite-settings" data-testid="settings-kite-section">
    <!-- OAuth section -->
    ...
    <!-- Tier 3: Personal Kite Connect app credentials -->
    <div class="api-creds-section">
      <p class="api-creds-label">Kite Connect App Credentials (optional)</p>
      <p class="form-hint" style="margin-bottom: 12px;">
        Save your own Kite Connect app key + secret. Required if you want to use your personal Kite Connect app instead of the platform's.
        Create an app at <a href="https://developers.kite.trade" target="_blank" rel="noopener" class="link">developers.kite.trade</a>.
      </p>
      ...
      <template v-else-if="saved?.has_credentials && !isEditing">
        ...
        <p v-if="credsError" class="error-msg" data-testid="settings-kite-creds-error">{{ credsError }}</p>
        <p v-if="credsSuccess" class="success-msg">{{ credsSuccess }}</p>

        <div class="btn-row">
          <button @click="startEdit" class="btn btn-outline" data-testid="settings-kite-creds-edit-btn">Edit</button>
          <button
            @click="handleCredsDelete"
            :disabled="credsDeleting"
            class="btn btn-danger"
            data-testid="settings-kite-creds-delete-btn"
          >
      ...
      <template v-else>
        <div v-if="!saved?.has_credentials" class="no-creds-notice">
          No Kite Connect app credentials saved.
        </div>

        <div class="form-group">
          <label class="form-label" for="kite-api-key">API Key</label>
          <input
            id="kite-api-key"
            ...
            class="form-input"
            data-testid="settings-kite-api-key"
          />
        </div>

        <div class="form-group">
          <label class="form-label" for="kite-api-secret">API Secret</label>
          <input
            id="kite-api-secret"
            ...
            class="form-input"
            data-testid="settings-kite-api-secret"
          />
        </div>
        ...
        <div class="btn-row">
          <button
            ...
            class="btn btn-primary"
            data-testid="settings-kite-creds-save-btn"
          >
          ...
          <button
            ...
            data-testid="settings-kite-creds-cancel-btn"
          >
            Cancel
          </button>
        </div>
